...
const makeEmulatedSafeProvider: any = ({
  ...
  return {
    ...
    send(rpcData: any, callback: any) {
      const { id, jsonrpc, method, params } = rpcData
      ...
      if (method === 'eth_sendTransaction') {
        const [{ from, to, gasPrice, value, data, nonce }] = params

        if (from.toLowerCase() !== safeAddress.toLowerCase()) {
          return callback(
            new Error(`expected to be from safe address ${safeAddress} but got ${from}`)
          )
        }
            ...
            params: [
              {
                ...
                value,
                nonce,
                data: safeMasterCopy.contract.methods
                  .execTransaction(
                    to,
                    value || 0,
                    data,
      ...
      if (method === 'eth_estimateGas') {
        const [{ from, to, gas, gasPrice, value, data, nonce }] = params
      ...
      if (method === 'gs_multi_send') {
        ...
        const callData = multiSend.contract.methods
          .multiSend(
            `0x${params
              .map((tx: Transaction) =>
                [
